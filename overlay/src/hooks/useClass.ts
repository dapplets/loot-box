...
export default function useClass(activeTokenType:number,creationForm:any,activeDropType:any,DropType:any,nodeFrom:any,nodeTo:any,nodeTokenAmount:any,onLink:(x:boolean)=>void,activeDropTypeFt:any,DropTypeFt:any,check:any, link:boolean,nodeDropAmount:any,checkFT:()=>boolean|undefined,nodeTokenContract:any,value:any,nodeDropChance:any,setDropType:any,booleanNodeTokenAmount:boolean|undefined, booleanNodeTokenContract:boolean|undefined, booleanNodeTokenTicer:any,
booleanNodeFrom:any,
booleanNodeTo:any,
    ...
    const LinkBlock = useMemo(() => {
        if (activeTokenType === 0) {
          ...
          if (
            ...
            creationForm.ftContentItems[0].contractAddress.length >= 1  ?  nodeDropAmount.current?.classList.remove('invalid') && nodeTokenContract.current?.classList.add('invalid') : nodeTokenContract.current?.classList.remove('invalid')
            onLink(true);
